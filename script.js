...
// Prevent selecting both main/specific
...
function formatStudentId(){
  ...
  let f=val.slice(0,4); if(val.length>4) f+='-'+val.slice(4,6); if(val.length>6) f+='-'+val.slice(6,11);
  studentId.value=f; checkStudent(); updateSubjects(); toggleBorrowButton(); checkAdmin();
}
...
function checkStudent(){
  ...
  if(students[id]){
    ...
    sName.innerText="-"; sCourse.innerText="-"; sYear.innerText="-"; borrowTime.innerText="-";
  }
}
...
function toggleBorrowButton(){
  ...
  const ready=studentId.value.trim() && p && subjectSelect.value && getSelectedLabRoom() && equipSelect.value && (students[studentId.value.trim()] || studentId.value.trim()==="3636-36-36363");
...
function refreshEquipmentUI(){
  equipSelect.innerHTML=""; returnSelect.innerHTML="";
...
borrowBtn.addEventListener("click", ()=>{
  ...
  transactions.push({ tx:transactionCounter++, studentID:id, student:s?s.name:"Admin", course:s?s.course:"Admin", year:s?s.year:"Admin", item:item.name, code:item.code, purpose:p, subject:subj, lab:lab, borrowTime:now.toLocaleString(), duration:dur, returnTime:"-", status:"Borrowed", condition:"-", penalty:0, deadline:deadline });
  saveData(); refreshTables(); alert(`Success! ${item.name} (${item.code}) borrowed by ${s?s.name:"Admin"}`);
});
...
returnBtn.addEventListener("click", ()=>{
  ...
  const now=new Date(); item.status="Available";
...
function refreshTables(){
  ...
  refreshEquipmentUI(); toggleBorrowButton(); refreshAdminTransactions(); refreshAdminEquipList();
}
...
// ADMIN CHECK
function checkAdmin(){ adminPanel.style.display = studentId.value.trim()==="3636-36-36363"?"block":"none";}

// ADMIN EQUIPMENT
function refreshAdminEquipList(){ allEquipList.innerHTML=""; equipment.forEach((e,idx)=>{ allEquipList.innerHTML+=`<li>${e.name} (${e.code}) - Status: ${e.status} <button onclick="editEquip(${idx})">Edit</button> <button onclick="deleteEquip(${idx})">Delete</button></li>`;});}
...
addEquipBtn.addEventListener("click", ()=>{
  ...
  equipment.push({id, code, name, status:"Available"}); saveData(); refreshEquipmentUI(); refreshAdminEquipList(); newEquipName.value=""; newEquipCode.value=""; alert(`${name} (${code}) added!`);
});
